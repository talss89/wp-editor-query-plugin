...
This plugin provides a number of mechanisms to achieve this:

1. Marking CSS with `@media (wp-editor)`, or `@editor`
2. 'Adopting' an entrypoint with the loader option `adopt`.

You can combine approaches, if that's useful.

The resulting CSS file(s) can then be loaded into WordPress via `add_editor_style()`.
